		...
		<div id="mypage">
			...
			<div class="mypage_content">
				...
				<div class="inq_list">
					...
					<a class="inquering btn_c" href="${contextPath}/service/inqForm.do?id=${log_id}">문의하기</a>
								...
								<c:when test="${totArticles > 100}">
									<c:forEach var="page" begin="1" end="${endPage}" step="1">
										<c:if test="${section > 1 && page == 1}">
											...
											<li><a href="${contextPath}/notice/noticeView.do?section=${section-1}&pageNum=${currentPage}"> prev</a></li>
										</c:if>
										<c:choose>
											<c:when test="${pageNum==page}">
												<li><a class="selPage" href="${contextPath}/notice/noticeView.do?section=${section}&pageNum=${page}">${(section-1)*10+page}</a></li>
												<c:set var="currentPage" value="${pageNum}" scope="application"/>
											</c:when>
											<c:otherwise>
												<li><a class="noLine" href="${contextPath}/notice/noticeView.do?section=${section}&pageNum=${page}">${(section-1)*10+page}</a></li>
											</c:otherwise>
										</c:choose>
										<c:if test="${page == 10 && totArticles/100>section}">
											<!-- <a href="${contextPath}/board/listArticles.do?section=${section+1}&pageNum=1"> next</a> -->
											<li><a href="${contextPath}/notice/noticeView.do?section=${section+1}&pageNum=${currentPage}"> next</a></li>
										</c:if>
									</c:forEach>
								</c:when>
								<c:when test="${totArticles <= 100}">
									...
									<c:forEach var="page" begin="1" end="${totArticles/10+1}" step="1">
										<c:choose>
											<c:when test="${page == pageNum}">
												<li><a class="selPage" href="${contextPath}/notice/noticeView.do?section=${section}&pageNum=${page}">${page}</a></li>
											</c:when>
											<c:otherwise>
												<li><a class="noLine" href="${contextPath}/notice/noticeView.do?section=${section}&pageNum=${page}">${page}</a></li>
											</c:otherwise>
										</c:choose>
									</c:forEach>
								</c:when>
